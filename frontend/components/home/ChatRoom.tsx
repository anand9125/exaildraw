...
const ChatRoom = ({ jwtCookie }: { jwtCookie: RequestCookie }) => {
  const activeRoom = useAppSelector((state:any) => state.app.activeRoom);
  const userState = useAppSelector((state:any) => state.app.user);
  ...
  const { socket, isLoading, isError } = useWebSocket(
    `${NEXT_PUBLIC_WS_URL}?token=${jwtCookie.value}`
  );
