...
export async function signupAction(
  ...
  const rawFormData = {
    ...
    password: `${formData.get("password")}`,
      ...
      console.log(axiosInstance.defaults.baseURL,"this is url");
  ...
  try {
    const res = await axiosInstance.post("/user/register", validatedFields.data);
    ...
    console.log(error);
    console.log((error as any).response),"thsi is problem";
...
export async function signinAction(
  ...
  try {
    const res = await axiosInstance.post("/user/login", validatedFields.data);
    if (res.data.token) {
      (await cookies()).set("jwt", res.data.token, {
        httpOnly: true,
        maxAge: 30 * 24 * 60 * 60 * 1000,
